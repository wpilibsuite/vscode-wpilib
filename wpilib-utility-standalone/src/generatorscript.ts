...
async function handleCppCreate(_item: IDisplayJSON, _srcRoot: string): Promise<void> {
  ...
  const result = await generateCopyCpp(resourceRoot, templateFolder, path.join(gradleRoot, _item.gradlebase), toFolder, false, false, false);
  if (!result) {
...
async function handleJavaCreate(_item: IDisplayJSON, _srcRoot: string): Promise<void> {
  ...
  const result = await generateCopyJava(resourceRoot, templateFolder, path.join(gradleRoot, _item.gradlebase), toFolder,
                                        'frc.robot.Robot', path.join('frc', 'robot'), false, false);
